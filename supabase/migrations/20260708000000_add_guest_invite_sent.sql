...
create or replace function public.update_guest_details(
  ...
  guest_couple_members jsonb,
  guest_max_guests integer,
...
begin
  ...
  set name=trim(guest_name),
      invite_type=guest_invite_type,
      couple_members=case when guest_invite_type='couple' then guest_couple_members else null end,
      max_guests=guest_max_guests,
      invite_sent=coalesce(guest_invite_sent, false)
...
begin
  if not public.is_admin() then raise exception 'Acesso administrativo necessário' using errcode = '42501'; end if;
  update public.guests
  set invite_sent=coalesce(next_invite_sent, false)
  where id=target_guest_id returning * into updated_guest;
  if updated_guest.id is null then raise exception 'Convidado não encontrado' using errcode = 'P0002'; end if;
  return updated_guest;
end; $$;
...
revoke all on function public.update_guest_details(uuid,text,text,jsonb,integer,boolean),public.set_guest_invite_sent(uuid,boolean) from public,anon;
grant execute on function public.update_guest_details(uuid,text,text,jsonb,integer,boolean),public.set_guest_invite_sent(uuid,boolean) to authenticated;
